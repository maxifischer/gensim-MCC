...
word2Id = pickle.load(open('data/tok2idx.pkl', 'rb'))
wordlist = pickle.load(open('data/idx2tok.pkl', 'rb'))
...
times = range(0,5) # total number of time points (20/range(27) for ngram/nyt)
...
nn = 50
nc = 5
emb = pickle.load(open("runs/L10T50G100A1ngU_iter4.p", "rb"))[4] #emb_all['U_%d' % times.index(199)]
...
v = emb[word2Id[word],:]
for year in times:
    emb = pickle.load(open("runs/L10T50G100A1ngU_iter{0}.p".format(year), "rb"))[4] #emb_all['U_%d' % times.index(year)]
...
#plt.show()
plt.savefig('tsne_output/tsne_{0}.png'.format(word))
...
plt.plot(traj[:,0],traj[:,1])
#plt.show()
plt.savefig('tsne_output/tsne_traj.png')
