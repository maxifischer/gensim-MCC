...
def getmat(f,v,rowflag):
    ...
    print(v)
    #if data.shape[0] < v:
    #    data = np.pad(data.todense(), (0, v-data.shape[0]), '')
    #print(data.shape)
    if rowflag: 
        X = ss.csr_matrix(data)
        #X = X[inds,:]
    else:
        X = ss.csc_matrix(data)
        #X = X[:,inds]

    print(X.shape)
    if X.shape[0] < v:
        #data = np.pad(X.todense(), (0, v-X.shape[0]), '')
        res = np.zeros((v, v))
        res[:X.shape[0], :X.shape[1]] = X.todense()
        X = res

    print(X.shape)

    return ss.csr_matrix(X) #X.todense()
...
def getclosest(wid,U, nn=10):
    C = []
    print(len(U))
    for t in range(len(U)):
        temp = U[t]
        #print(temp.shape)
        #print(temp[wid].shape)
